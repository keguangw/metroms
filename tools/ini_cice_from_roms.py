...
icemask   = np.where((hice_r*mask_r) > 0.01, 1, 0)
hice_r[hice_r <= 0.01] = 0.0
aice_r[hice_r <= 0.01] = 0.0
age_r[hice_r <= 0.01] = 0.0
hsno_r[hice_r <= 0.01] = 0.0
...
for n in range(len(NCAT[1:])):
    ...
    vicen_c[n,:,:] = np.where(np.logical_and( hice_r > NCAT[n], hice_r < NCAT[n+1] ), \
                              hice_r,0.0) * aicen_c[n,:,:] * mask_r
    vsnon_c[n,:,:] = np.where(np.logical_and( hice_r > NCAT[n], hice_r < NCAT[n+1] ), \
                              hsno_r,0.0) * aicen_c[n,:,:] * mask_r
    agen_c[n,:,:]  = np.where(np.logical_and( hice_r > NCAT[n], hice_r < NCAT[n+1] ), \
                              age_r, 0.0) * mask_r
    Tsfcn_c[n,:,:] = -1.8 - hice_r*10.0
    ...
    for k in range(nilyr):
        ...
        set_to_val(nc_cice,'qice0'+ch,icemask,-2.0e8,n)
